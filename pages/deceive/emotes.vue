<template>
  <div class="deceive-skin-list-header-wrapper">
    <div class="deceive-skin-list-header-title-wrapper">
      <h1 class="deceive-skin-list-header-title">Emotes / Cheers</h1>
    </div>
  ...
  <div class="deceive-skin-list-wrapper">
    <div
      ...
      v-for="item of emotelist.emotes"
      :key="item"
    >
...
import { type IEmotesListInterface } from "~/interfaces/Deceive/emotesInterface";
...
const emotelist: Ref<IEmotesListInterface> = ref({ emotes: [] });
const selectedAgent: Ref<string> = ref("");

function GenerateAgentSkinRouterLink(item_in: string) {
  const returnRouterLink = "/deceive/emote?id=" + item_in;
...
function getAgentSkinListImageUrl(name: string) {
  const dynamicImageUrl = new URL(
    `../../assets/images/Deceive/shopitems/emotes/${name}_small.webp`,
    import.meta.url
  ).href;
...
onMounted(() => {
  ...
  if (window.location.pathname === "/deceive/emotes") {
    const searchParams = new URLSearchParams(window.location.search);
    selectedAgent.value = searchParams.get("agent") || "";

    if (selectedAgent.value !== "") {
      const agentSkinArray = Object.entries(EmotesData.emoteskinlist);
      for (const [agentName, skins] of agentSkinArray) {
        if (selectedAgent.value == "ALL" && skins.skins.length !== 0) {
          console.log(skins.skins);
          emotelist.value.emotes = [...emotelist.value.emotes, ...skins.skins];
        } else if (agentName === selectedAgent.value) {
          emotelist.value.emotes = skins.skins;
          console.log("ACHTUNG");
        }
      }
    }
