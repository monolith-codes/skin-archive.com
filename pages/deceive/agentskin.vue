...
import { type IAgentSkinsInterface } from "~/interfaces/Deceive/agentskinsInterface";
...
function getAgentSkinImageUrl(name: string) {
  const dynamicImageUrl = new URL(
    `../../assets/images/Deceive/shopitems/agentskins/${name}_big.webp`,
    import.meta.url
  ).href;
...
function getAgentSkinVariantImageUrl(name: string) {
  const dynamicImageUrl = new URL(
    `../../assets/images/Deceive/shopitems/agentskins/${name}_V1_big.webp`,
    import.meta.url
  ).href;
  return dynamicImageUrl;
}
...
onMounted(() => {
  ...
  if (window.location.pathname === "/deceive/agentskin") {
    const searchParams = new URLSearchParams(window.location.search);
    selectedSkin.value = searchParams.get("id") || "";

    if (selectedSkin.value !== "") {
      const agentSkinArray = Object.entries(AgentSkinData.skinlist);
